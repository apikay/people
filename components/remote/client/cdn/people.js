...
(function() {
  ...
  var People = function(options) {
    ...
    if (this.getInfo() === null) {
      this.ajax('GET', this.url + '/remote/info/', {}, function (response) {
   ...
   * @returns {string}
   */
  People.prototype.getCookie = function (nameArg) {
    var name = nameArg + '=';
  ...
  People.prototype.errors = function(data, messege) {
    ...
    if (data.success === true) {
      ...
      for (var key in data.errfor) {
        if (data.errfor.hasOwnProperty(key)) {
    ...
    document.addEventListener('click', function (e) {
      ...
      switch (e.target.id) {
        ...
        case 'ppl-update-profile-btn':
          elms = document.querySelectorAll('#ppl-profile-form input');
          values = {fields: {}};
          for (var i = 0, elmProfile; (elmProfile = elms[i]); ++i) {
            if (elmProfile.name === 'username' || elmProfile.name === 'email') {
              values[elmProfile.name] = elmProfile.value;
            }
            else {
              values.fields[elmProfile.name] = elmProfile.value;
            }
          }
        ...
        case 'ppl-update-password-btn':
          elms = document.querySelectorAll('#ppl-new-password input');
          values = {};
          for (var j = 0, elmPass; (elmPass = elms[j]); ++i) {
            values[elmPass.name] = elmPass.value;
          }
